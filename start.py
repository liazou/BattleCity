...
def bullet_collide_block(bullet, map_stage):
	# 子弹碰撞石头墙
	'''					
	if pygame.sprite.spritecollide(bullet, map_stage.brickGroup, True, None):
		bullet.being = False
	'''
	# 等价方案(更科学点)
	...
	'''
	if bullet.stronger:
		if pygame.sprite.spritecollide(bullet, map_stage.ironGroup, True, None):
			bullet.being = False
	else:
		if pygame.sprite.spritecollide(bullet, map_stage.ironGroup, False, None):
			bullet.being = False
	'''
	# 等价方案(更科学点)
...
def main():
	...
	add_sound.set_volume(sound_volumne)
	bang_sound = pygame.mixer.Sound("audios/bang.wav")
	bang_sound.set_volume(sound_volumne)
	blast_sound = pygame.mixer.Sound("audios/blast.wav")
	blast_sound.set_volume(sound_volumne)
	fire_sound = pygame.mixer.Sound("audios/fire.wav")
	fire_sound.set_volume(sound_volumne)
	Gunfire_sound = pygame.mixer.Sound("audios/Gunfire.wav")
	Gunfire_sound.set_volume(sound_volumne)
	hit_sound = pygame.mixer.Sound("audios/hit.wav")
	hit_sound.set_volume(sound_volumne)
	start_sound = pygame.mixer.Sound("audios/start.wav")
	start_sound.set_volume(sound_volumne)
	# 开始界面
	...
	while not is_gameover:
		...
		# 该关卡坦克总数量
		enemytanks_remaining = min(10 + stage * 5, 100)
		# 场上存在的敌方坦克总数量
		enemytanks_now = 0
		# 场上可以存在的敌方坦克总数量
		enemytanks_now_max = min(max(stage * 2, 4), 10 + stage // 10)
		# 精灵组
		...
		# 	-钢枪大本营恢复事件
		noprotectHomeEvent = pygame.constants.USEREVENT + 3
		pygame.time.set_timer(noprotectHomeEvent, 8000)
		# 关卡地图
		...
		while True:
			if is_gameover is True:
				break
			if enemytanks_remaining < 1 and enemytanks_now < 1:
				is_gameover = False
				break
			for event in pygame.event.get():
				if event.type == pygame.QUIT:
				...
				if event.type == noprotectHomeEvent:
					if myhome.protected:
						map_stage.remove_home_protection()
						myhome.protected = False
			# 检查用户键盘操作
			...
			for tank_player in mytanksGroup:
				if tank_player.bullet.being:
					...
					bullet_collide_block(tank_player.bullet, map_stage)
					# 子弹碰大本营
						...
						if each.bullet.being:
							...
							for tank_player in mytanksGroup:
								if pygame.sprite.collide_rect(each.bullet, tank_player):
									...
									enemybulletsGroup.remove(each.bullet)
									break
							bullet_collide_block(each.bullet, map_stage)
							# 子弹碰大本营
